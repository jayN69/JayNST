...
metadata {
    definition (name: "Heiman SmartPlug", namespace: "JayN", author: "SmartThings/JayN", ocfDeviceType: "oic.d.smartplug", mnmn: "SmartThings",  vid: "generic-switch-power-energy") {
...
def parse(String description) {
    ...
    if (event) {
        ...
        if (event.name == "switch" && !descMap.isClusterSpecific && descMap.commandInt == 0x0B) {
            ...
            return [:]
        } else if (event.name== "power") {  //소비전력(W) 전력이 3자리여서 나누기 10을 함
            event.value = event.value/getPowerDiv()
            event.unit = "W"
        } else if (event.name== "energy") { //에너지 소비량(kWh)  다원처럼 1000을 나눔
...
private int getPowerDiv() {    //소비전력(w)
    isHeimanOutlet() ? 10 : 1  //Heiman이면 나누기 10을 한다. 전력이 3자리로 올라온다.(event outer:[name:power, value:746, unit:W])
}
...
private boolean isHeimanOutlet() {
    device.getDataValue("manufacturer") == "Heiman"
}